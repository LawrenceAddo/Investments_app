...
class Investment extends Component {
  ...
  investmentsList = async (userId) => {
    try {
      const res = await Axios.get(`/api/investments/${userId}`)
      const data = res.data
      this.setState({ investments: data, userId })
    } catch (err) {
      console.log(err)
    }
  };
  ...
  companiesList = async (companyId) => {
    try {
      const res = await Axios.get(`/api/companies/${companyId}`)
      const data = res.data
      this.setState({ companies: [data], companyId })
    } catch (err) {
      console.log(err)
    }
  };
  ...
  render() {
    const investUser = this.state.investments.filter(
      (investment) => investment._userId === this.state.userId
    )
